    ...
    columns: [
        ...
        {data: 'category_slug',title: 'Category Slug'},
        {
            ...
            render: function (val, type, doc) {
                var pathEdit = Router.routes.AdminBlogCategoryEdit.path({_id: doc._id});
                var text = '<a href="' + pathEdit + '" class="edit-category btn btn-success"><i class="fa fa-edit"></i></a>';
                text += '<a href="#" data-id="' + doc._id + '" class="delete-category btn btn-danger"><i class="fa fa-remove"></i></a>';
                return text;
            }
        }
    ]
    ...
    columns: [
        {
            ...
            render: function(val, type, doc) {
                ...
                text += '</div>';
                text += '<div>';
                text += '<a href="'+Router.routes.BlogDetail.path({post_slug: doc.post_slug, category_slug: BlogCategories.findOne(doc.post_category).category_slug})+'" target="_blank">';
                text += 'ดูบทความ';
                text += '</a>';
                text += '</div>';
                return text;
        ...
        },
        {
            ...
            title: '', 
            render: function (val, type, doc) {
                var pathEdit = Router.routes.AdminBlogEdit.path({_id: doc._id});
                var text = '<a href="' + pathEdit + '" class="edit-blog btn btn-success"><i class="fa fa-edit"></i></a>';
                text += '<a href="#" data-id="' + doc._id + '" class="delete-blog btn btn-danger"><i class="fa fa-remove"></i></a>';
                return text;
            }
        }
    ]
...
TabularTables.MemberFarmCats = new Tabular.Table({
    ...
    columns: [
        ...
        },
        {
            ...
            title: '',
            render: function(val, type, doc){
                var pathEdit = Router.routes.MemberFarmCatEdit.path({_id: doc._id});
                var text = '<a href="' + pathEdit + '" class="edit-cat btn btn-success"><i class="fa fa-edit"></i></a>';
                text += ' <a href="#" data-id="' + doc._id + '" class="delete-cat btn btn-danger"><i class="fa fa-remove"></i></a>';
                return text;
            }
        }
    ]
